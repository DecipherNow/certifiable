...
post '/servers/:server' do
    `/usr/local/bin/server.sh #{params['server']} "#{params['cn']}" "#{params['program']}" #{SecureRandom.hex(3)} "#{params['san']}"`
end
...
post '/users/:user' do
    `/usr/local/bin/user.sh #{params['user']} "#{params['cn']}" "#{params['program']}" #{SecureRandom.hex(3)}`
end
